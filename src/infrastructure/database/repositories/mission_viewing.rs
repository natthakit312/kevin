...
impl MissionViewingRepository for MissionViewingPostgres {
    async fn get_mission_crew(&self, mission_id: i32) -> Result<Vec<BrawlerModel>> {
        ...
        let sql = r#"
                SELECT b.id::INTEGER AS id,
                        COALESCE(b.display_name, '') AS display_name,
                        COALESCE(b.avatar_url, '') AS avatar_url,
                        COALESCE(b.specialty, '') AS specialty,
                        COALESCE(s.success_count, 0)::INTEGER AS mission_success_count,
                        COALESCE(j.joined_count, 0)::INTEGER AS mission_join_count
    ...
    async fn view_detail(&self, mission_id: i32) -> Result<MissionModel> {
        ...
        let sql = r#"
            SELECT m.id,
                    ...
                    COUNT(cm.brawler_id)::BIGINT AS crew_count,
                    m.max_crew,
                    m.created_at,
                    m.updated_at
            FROM missions m
            ...
            WHERE m.deleted_at IS NULL
                AND m.id = $1
            GROUP BY m.id, b.display_name, m.name, m.description, m.status, m.chief_id, m.max_crew, m.created_at, m.updated_at
            LIMIT 1
        "#;
    ...
    async fn gets(&self, filter: &MissionFilter) -> Result<Vec<MissionModel>> {
        use diesel::sql_types::{BigInt, Nullable, Varchar};
        ...
        let limit_bind = filter.limit.unwrap_or(100);
        let offset_bind = filter.offset.unwrap_or(0);

        let rows = diesel::sql_query(sql)
            .bind::<Nullable<Varchar>, _>(status_bind)
            .bind::<Nullable<Varchar>, _>(name_bind)
            .bind::<BigInt, _>(limit_bind)
            .bind::<BigInt, _>(offset_bind)
